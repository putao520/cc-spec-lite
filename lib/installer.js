...
async function install(options = {}) {
  ...
  const isInteractive = process.stdout.isTTY && !process.env.CI && !process.env.NON_INTERACTIVE;
  ...
  if (currentLang && !options.force) {
    ...
    if (isInteractive) {
      ...
      // Non-interactive mode: keep existing language
      console.log(chalk.cyan('Non-interactive mode: keeping existing language\n'));
      options.lang = currentLang;
    }
  }
...
async function uninstall() {
  ...
  if (isInteractive) {
    ...
    confirmUninstall = answers.confirm;
  } else {
    // Non-interactive mode: require --force flag or fail
    if (!process.env.FORCE_UNINSTALL) {
      console.log(chalk.red('\nUninstall requires confirmation in non-interactive mode.'));
      console.log(chalk.cyan('Set FORCE_UNINSTALL=1 environment variable to proceed.\n'));
      process.exit(1);
    }
  ...
  if (backupInfo && fs.existsSync(backupInfo.backup)) {
    ...
    let restoreBackup = true;  // default to restore
    if (isInteractive) {
      ...
      console.log(chalk.cyan('Non-interactive mode: restoring backup\n'));
    }
  ...
  if (aiwVersion) {
    ...
    if (comparison === -1) {
      ...
      if (isInteractive) {
        ...
        console.log(chalk.cyan('Non-interactive mode: skipping aiw update\n'));
      }
    ...
    if (isInteractive) {
      ...
      // Non-interactive mode: auto-install if --skip-aiw not set
      console.log(chalk.cyan('Non-interactive mode: installing aiw\n'));
      shouldInstall = true;
    }
  ...
  try {
    ...
    if (existingFiles.length > 0 && !force) {
      ...
      if (isInteractive) {
        ...
        // Non-interactive mode: skip copy
        console.log(chalk.cyan('Non-interactive mode: skipping role file copy\n'));
      }
