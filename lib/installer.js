...
async function install(options) {
  ...
  console.log(chalk.blue('  cc-spec-lite Installer v1.0.0'));
...
async function uninstall() {
  ...
  try {
    // Remove files but keep backup
    const files = ['CLAUDE.md', 'skills', 'commands', 'scripts', 'roles', 'hooks'];
 ...
 * Install aiw
 */
async function installAIW() {
  const spinner = ora('Installing aiw...').start();

  try {
    execSync('npm install -g @putao520/agentic-warden', { stdio: 'pipe' });
    spinner.succeed('aiw installed successfully\n');
  } catch (error) {
    spinner.fail('aiw installation failed');
    console.error(chalk.red('Error:'), error.message);
    console.log(chalk.yellow('\nPlease install manually:'));
    console.log('  npm install -g @putao520/agentic-warden\n');
    process.exit(1);
  }
}
