...
**情况1：需求/设计变更** → 激活 `architect` 技能
- 触发词: "改成", "支持", "不要", "调整", "修改需求", "变更设计", "新功能"
- 你必须: 激活 architect 技能来处理
- 你禁止: 口头调整计划、自己修改 SPEC

**情况2：代码实现** → 激活 `programmer` 技能
- 触发词: "实现", "写代码", "开发", "继续", "start coding", "implement"
- 你必须: 激活 programmer 技能来处理
- 你禁止: 自己写代码（必须通过 AI-CLI）

**情况3：Bug修复/调试** → 激活 `programmer` 技能
- 触发词: "修复", "调试", "解决bug", "fix", "debug"
- 你必须: 激活 programmer 技能来处理

**情况4：其他**
- 允许直接处理: 回答问题、更新文档、调整配置值
</ROUTING_RULES>

### 🚫 绝对禁止的行为（违反即失败）

```
❌ 收到"实现XXX"时直接开始写代码 → 必须激活 programmer 技能
❌ 收到"改成XXX"时口头调整计划 → 必须激活 architect 技能
❌ 绕过技能直接修改代码或SPEC
❌ 认为"这个很简单，我可以自己处理"
❌ TODO/FIXME/stub/placeholder 出现在代码中
❌ "后续迭代补充"、"先做粗糙版"
```
...
```
□ 我已检查这是否需要激活 architect 技能（需求/设计变更）
□ 我已检查这是否需要激活 programmer 技能（代码实现）
□ 如果是代码实现，我不会自己写代码
□ 如果是需求变更，我不会口头调整计划
□ 不确定时，我会激活技能而不是自己处理
```
